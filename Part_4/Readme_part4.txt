...
consent forms in the folder 'part4_dataset' as a pdf file named "Consent forms HCIRA Project 1 part4" that we have submitted on canvas. We have not used their names to store
the gestures so it is not possible to tell which user has drawn the corresponding set of gestures. In the pdf file, we have attached the image that we used in while
Collecting the data to make sure that the users know how to draw each and every gesture

d(Submit Full Dataset) : The entire XML file dataset is present in the 'user_logs' folder that is present in the 'part4_dataset' zip folder that we submitted. Each and every user has drawn 10 examples
